...
class adminusers extends manage {
	...
	function add() {
		...
		sql::query("INSERT INTO prname_rt(super, aid) VALUES(0, ".$aid.")");
	...
	function printEdit() {
		...
		if ($_SESSION['admin_status'] > $pStatus) {
	...
	function edit() {
		...
		if ($password == "") {
			$sql = "UPDATE prname_sadmin SET `admin_name`='".$name."', `status`=".$status.", `enabled`=1, `canedit`='".$canedit."' WHERE `admin_id`=".$parent;
		}
		else {
			$password = md5(base64_encode($config['md5'].$password));

			$sql = "UPDATE prname_sadmin SET `admin_name`='".$name."', `admin_password`='".$password."', `status`=".$status.", `enabled`=1, `canedit`='".$canedit."' WHERE `admin_id`=".$parent;
		}
	...
	function showHide() {
		...
		$pStatus = $info['status'];
		$pName = $info['admin_name'];
		if ($_SESSION['admin_status'] > $pStatus || $pName == $_SESSION['admin_name']) {
			header("Location: /manage/users/_aerror_b1");
			return;
		}			
